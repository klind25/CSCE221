...
private:
    ...
    void upheap(size_type index) {
        ...
        // Parent > child
        if (comp(c[index], c[parent(index)])) {
            return;
        }
        // Recursive case
    ...
    void downheap(size_type index) {
        ...
        // current > children
        size_type maxChildIndex = left_child(index);
        if (right_child(index) < c.size() && comp(c[maxChildIndex], c[right_child(index)])) {
            maxChildIndex = right_child(index);
        }
        // Current > maxchild
        if (comp(c[maxChildIndex], c[index])) {
            return;
        }
        // Recursive
        std::swap(c[index], c[maxChildIndex]);
