...
std::list<value_type<T>> topologicalSort(const WeightedGraph<T>& graph)
{
    ...
    std::list<value_type<T>> list = {};

    computeIndegrees(graph, indegrees);

    std::queue<value_type<T>> queue = {};
    
    for (const auto& [first, adj_list] : graph) {
        if (indegrees[first] == 0) {
            queue.push(first);
        }
    }
    ...
    while (!queue.empty()) {
        ...
        for (auto iter = graph.begin(vertex); iter != graph.end(vertex); ++iter) {
            auto [second, weight] = *iter;
            --indegrees[second];
...
std::list<value_type<T>> dijkstrasAlgorithm(const WeightedGraph<T>& graph, vertex_type<T> initial_node, vertex_type<T> destination_node)
{
    ...
    for (const auto& [vertex, adj] : graph) {
        q.push(vertex);
    }

    while(!q.empty()) {
        value_type<T> vertex = q.top();
        q.pop();
        ...
        for (auto iter = graph.begin(vertex); iter != graph.end(vertex); ++iter) {
            ...
            if (relax<T>(vertex, second, weight, distances, predecessors)) {
    ...
    std::list<value_type<T>> shortest = {};
    value_type<T> node = destination_node;

    while (predecessors[node] != std::nullopt) {
        shortest.push_front(node);
        node = *predecessors[node];
    }

    if (shortest.empty() && (initial_node != destination_node)) {
        return shortest;
    }

    shortest.push_front(initial_node);
